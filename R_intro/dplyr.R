...
c <- select(clinical, tumor_stage, ethnicity, disease)
...
race_BRCA <- filter(clinical, disease == 'BRCA')
#### Combining commands ####
...
# years_smoked, and year_of_birth from the object clinical 
# for only living patients (vital_status) who have smoked fewer than 1 cigarettes per day
...
## and create a new column called total_cig representing an estimate 
## of the total number of cigarettes smoked 
## (use columns years smoked and cigarettes per day)

clinical %>% 
  filter(disease == 'LUSC') %>%
  mutate(total_cig = years_smoked * 365 * cigarettes_per_day)
...
## Challenge: create object called smoke_complete from clinical 
## that contains no missing data for cigarettes per day or age at diagnosis 
...
## Challenge: create a new object called birth_complete 
## that contains no missing data for year of birth or vital status
